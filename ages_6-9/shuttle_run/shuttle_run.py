...
# Source: ages_6-9/shuttle_run/shuttle_run.ipynb
...
def count_laps(lms, lap_count, state, left_line_x, right_line_x):
    ...
    max_foot_x = max(lms[29].x, lms[31].x, lms[30].x, lms[32].x)
    min_foot_x = min(lms[29].x, lms[31].x, lms[30].x, lms[32].x)
...
def shuttle_time_score(time_seconds):
    
    if time_seconds <= 13.5:
        return 6
    elif 13.6 <= time_seconds <= 14.2:
        return 5
    elif 14.3 <= time_seconds <= 14.9:
        return 4
    elif 15.0 <= time_seconds <= 15.6:
        return 3
    elif 15.7 <= time_seconds <= 16.3:
        return 2
    else:
        return 1
...
def compute_movement_metrics(hip_x_list, hip_y_list, time_list):
    ...
    if len(peaks) > 1:
        ...
        speed_var = safe_std(stride_intervals, default=0.2)
    else:
        speed_var = 0.2  # fallback for low stride count

    # -------- SWAY (Side-to-side control) --------
    sway = safe_std(hip_x_arr, default=0.0)
    ...
    if len(t_arr) > 2:
        ...
        valid = time_deltas != 0
        if not np.any(valid):
            return speed_var, sway, 1.0
        velocities = np.diff(hip_y_arr)[valid] / time_deltas[valid]
        accel = np.diff(velocities)
        accel_var = safe_std(accel, default=1.0) if len(accel) > 1 else 1.0
    else:
        accel_var = 1.0

    return speed_var, sway, accel_var
...
ensure_csv_header(
    ...
    ["ID", "Name", "Video_path", "Accurate_lap_count", "Partial_lap_count", "Avg_turn_angle", "Movement:speed_var", "Movement:sway", "Movement:accel_var", "Avg_turn_time", "Score_18", "Category"],
)

# open("shuttle_run_results.csv", "w").close()
# data = ["ID", "Name", "Video_path", "Accurate_lap_count", "Partial_lap_count", "Avg_turn_angle", "Movement:speed_var", "Movement:sway", "Movement:accel_var", "Avg_turn_time", "Score_18", "Category"]
# add_data(data)
...
def shuttle_run(ID="DCXXXX", name="Life", path=0, data_print='Y'):
    ...
    # A. lap-time score
    lap_time_score = shuttle_time_score(time_seconds)
    ...
    # adding data
    data = [ID, name, path, lap_count, partial_lap_count, avg_turn_angle, speed_var, sway, accel_var, avg_turn_time, _score, category]
    add_data(data)
