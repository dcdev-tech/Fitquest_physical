...
jumped_distance_cm: float | None = None
if selected_activity == "long_jump":
    jumped_distance_cm = st.number_input(
        "Jumped Distance (cm)",
        min_value=0.0,
        value=60.0,
        step=1.0,
        key="jumped_distance_cm",
        help="Enter the measured jump distance in centimeters.",
    )
...
payload = {
    ...
    "duration": duration,
    "jumped_distance_cm": jumped_distance_cm,
}
...
uploadBtn.onclick = async () => {
  try {
    ...
    if (cfg.activity === 'long_jump' && cfg.jumped_distance_cm != null) {
      form.append('jumped_length', String(cfg.jumped_distance_cm));
    }
...
st.subheader("Step 4: Or Upload Existing Video")
        ...
        try:
            ...
            def run_upload() -> None:
                try:
                    ...
                    if selected_activity == "long_jump" and jumped_distance_cm is not None:
                        data["jumped_length"] = str(float(jumped_distance_cm))
